...
#pragma GCC diagnostic pop
	...
	*/
	...
	switch (Status) {
		case 0x01:
			PRINTF(PSTR("alignment fault\n"));
			break;
		case 0x04:
			PRINTF(PSTR("instruction cache maintenance fault\n"));
			break;
		case 0x0C:
			PRINTF(PSTR("1st level translation, synchronous external abort\n"));
			break;
		case 0x0E:
			PRINTF(PSTR("2nd level translation, synchronous external abort\n"));
			break;
		case 0x05:
			PRINTF(PSTR("translation fault, section\n"));
			break;
		case 0x07:
			PRINTF(PSTR("translation fault, page\n"));
			break;
		case 0x03:
			PRINTF(PSTR("access flag fault, section\n"));
			break;
		case 0x06:
			PRINTF(PSTR("access flag fault, page\n"));
			break;
		case 0x09:
			PRINTF(PSTR("domain fault, section\n"));
			break;
		case 0x0B:
			PRINTF(PSTR("domain fault, page\n"));
			break;
		case 0x0D:
			PRINTF(PSTR("permission fault, section\n"));
			break;
		case 0x0F:
			PRINTF(PSTR("permission fault, page\n"));
			break;
		case 0x08:
			PRINTF(PSTR("synchronous external abort, nontranslation\n"));
			break;
		case 0x16:
			PRINTF(PSTR("asynchronous external abort\n"));
			break;
		case 0x02:
			PRINTF(PSTR("debug event.\n"));
			break;
		default:
			PRINTF(PSTR("undefined Status=%02X\n"), Status);
			break;
	}
	//	unsigned i;
	//	for (i = 0; i < 8; ++ i)
...
__STATIC_FORCEINLINE void L2_CleanDCache_by_Addr(volatile void *addr, int32_t dsize)
{
	if (dsize > 0) {
		...
		do {
			...
			L2C_310->CLEAN_LINE_PA =
				op_addr; // Atomic operation. These operations stall the slave ports until they are complete.
...
;===================================================================
	...
	ORR  r1,r1,#0x40                ;;; IRGN=b01 (inner cacheability for the translation table walk is Write-back
Write-allocate) ORR  r0,r0,r1 MCR  p15, 0, r0, c2, c0, 0      ;;; TTBR0
...
#define TTB_PARA_STRGLY                                                                                                \
	TTB_PARA(TEXval_STGORD, Bval_STGORD, Cval_STGORD, DOMAINval, 0 /* Shareable mot depend of this bit */, APRWval, 1)

//; setting for Outer and inner not cache normal memory
// not used
//#define	TTB_PARA_NORMAL_NOT_CACHE(ro, xn) TTB_PARA(TEXval_NOCACHE, Bval_NOCACHE, Cval_NOCACHE, DOMAINval, SHAREDval,
//(ro) ? APROval : APRWval, (xn) != 0)

//; setting for Outer and inner write back, write allocate normal memory (Cacheable)
//#define	TTB_PARA_NORMAL_CACHE       0b_0000_0001_1101_1110_1110
#define TTB_PARA_NORMAL_CACHE(ro, xn)                                                                                  \
	TTB_PARA(TEXval_WBCACHE, Bval_WBCACHE, Cval_WBCACHE, DOMAINval, SHAREDval, (ro) ? APROval : APRWval, (xn) != 0)

#define TTB_PARA_DEVICE                                                                                                \
	TTB_PARA(TEXval_DEVICE, Bval_DEVICE, Cval_DEVICE, DOMAINval, NoSHAREDval, APRWval, 1 /* XN=1   \
																											  */)
...
static uint32_t FLASHMEMINITFUNC ttb_accessbits(uintptr_t a, int ro, int xn)
{
...
static void FLASHMEMINITFUNC ttb_initialize(uint32_t (*accessbits)(uintptr_t a, int ro, int xn),
											uintptr_t textstart,
											uint_fast32_t textsize)
{
	extern volatile uint32_t __TTB_BASE; // получено из скрипта линкера
	volatile uint32_t *const tlbbase = &__TTB_BASE;
	unsigned i;
...
static void FLASHMEMINITFUNC ttb_map(uintptr_t va, /* virtual address */
...
static void FLASHMEMINITFUNC sysinit_vbar_initialize(void)
{
	...
	__set_MVBAR(vbase); // Set Monitor Vector Base Address Register (bits 4..0 should be zero) - на работу не вличет...
						// но на всякий случай
...
static void FLASHMEMINITFUNC sysinit_cache_L2_cpu0_initialize(void)
{
...
static void cortexa_cpuinfo(void)
{
	...
	PRINTF(PSTR("CPU%u: VBAR=%p, TTBR0=%p, cpsr=%08lX, SCTLR=%08lX, ACTLR=%08lX, sp=%08lX\n"),
		   ...
		   (unsigned long)__get_VBAR(),
		   (unsigned long)__get_TTBR0(),
		   (unsigned long)__get_CPSR(),
		   (unsigned long)__get_SCTLR(),
...
void Reset_CPUn_Handler(void)
{
	...
	sysinit_fpu_initialize();
	sysinit_vbar_initialize(); // interrupt vectors relocate
	sysinit_ttbr_initialize(); // TODO: убрать работу с L2 для второго процессора - Загрузка TTBR, инвалидация кеш
							   // памяти и включение MMU
	sysinit_cache_initialize(); // caches iniitialize
...
{
	return ((uint_fast16_t)buff[1] << 8) + ((uint_fast16_t)buff[0] << 0);
}

/* получить 24-бит значение */
uint_fast32_t USBD_peek_u24(const uint8_t *buff)
{
	return ((uint_fast32_t)buff[2] << 16) + ((uint_fast32_t)buff[1] << 8) + ((uint_fast32_t)buff[0] << 0);
}

/* получить 32-бит значение */
uint_fast32_t USBD_peek_u32(const uint8_t *buff)
{
	return ((uint_fast32_t)buff[3] << 24) + ((uint_fast32_t)buff[2] << 16) + ((uint_fast32_t)buff[1] << 8) +
...
{
	return ((uint_fast32_t)buff[0] << 24) + ((uint_fast32_t)buff[1] << 16) + ((uint_fast32_t)buff[2] << 8) +
...
{
	return ((uint_fast64_t)buff[0] << 56) + ((uint_fast64_t)buff[1] << 48) + ((uint_fast64_t)buff[2] << 40) +
		   ((uint_fast64_t)buff[3] << 32) + ((uint_fast64_t)buff[4] << 24) + ((uint_fast64_t)buff[5] << 16) +
		   ((uint_fast64_t)buff[6] << 8) + ((uint_fast64_t)buff[7] << 0);
}
